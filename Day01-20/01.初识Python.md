...
从官方网站的[下载页面](https://www.python.org/downloads/)找到下载链接并下载安装程序。需要注意，有些 Python 版本并没有提供 Windows 和 macOS 系统的安装程序，只提供了源代码的压缩文件；例如你想安装 Python 3.10，选择 3.10.10 或 3.10.11 就能找到 Windows 或 macOS 系统的安装包。

#### Windows环境

下面我们以 Windows 11 为例，讲解如何在 Windows 操作系统上安装 Python 环境。双击运行从官网下载的安装程序，会打开一个安装向导，按下面的清单勾选即可。

| 步骤 | 界面 | 勾选 / 填写 |
|---|---|---|
| 1 | 安装向导首页 | 勾选 `Add python.exe to PATH` · 勾选 `Use admin privileges when installing py.exe` · 选择 `Customize Installation`，不要用 `Install Now` |
| 2 | Optional Features | 全部勾选，其中第 2 项是包管理工具 pip，**必须勾**，然后点击 `Next` |
| 3 | Advanced Options | 勾选 `Add Python to environment variables` · 勾选 `Precompile standard library` · 把 `Customize install location` 改成自定义路径 |
| 4 | 安装中 | 等待完成，出现 `successful` 字样即为成功，失败会显示 `failed` |

> **路径要求**：自定义安装路径中**不应该包含中文、空格或其他特殊字符**。这一点会为你将来减少很多不必要的麻烦。

安装完成后，打开 Windows 的“命令行提示符”或 PowerShell，输入下面两条命令检查安装是否成功。
...
> **说明**：`python -V` 和 `pip -V` 是等价写法。两条命令都能打印出版本号，说明环境已经就绪。

如果安装过程报错或提示安装失败，很有可能是你的 Windows 系统缺失了一些动态链接库文件或缺少必要的构建工具导致的。可以在[微软官网](https://visualstudio.microsoft.com/zh-hans/downloads/)下载“Visual Studio 2022 生成工具”进行修复，也可以从[百度云盘](https://pan.baidu.com/s/1iNDnU5UVdDX5sKFqsiDg5Q)（提取码 `cjs3`）获取修复工具。该工具需要联网运行，勾选对应的选项进行修复，过程可能比较耗时，修复成功后可能会要求重启你的操作系统。

#### macOS环境

macOS 安装 Python 环境相较于 Windows 系统更为简单：从官方下载的安装包是一个 `pkg` 文件，双击运行之后不断地点击“继续”就安装成功了，几乎不用做任何的设置和勾选。

安装完成后，在 macOS 的“终端”工具中输入下面两条命令检查安装是否成功。
...
> **注意**：这里的命令是 `python3` 而不是 `python`，`pip` 同理。这一点和 Windows 不同，初学者最容易在这里卡住。
