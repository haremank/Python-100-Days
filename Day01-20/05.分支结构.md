...
Author: 骆昊
"""
height = float(input('身高(cm)：'))   # 输入: 175
weight = float(input('体重(kg)：'))   # 输入: 65
bmi = weight / (height / 100) ** 2
print(f'{bmi = :.1f}')   # 输出: bmi = 21.2
if 18.5 <= bmi < 24:
    print('你的身材很棒！')   # 输出: 你的身材很棒！
```
...
> **输入同上（175 / 65）时，输出的还是「你的身材很棒！」**。
> 想看到`else`分支的效果，把输入换成 `160` 和 `70`（BMI 约 `27.3`），
> 就会输出「你的身材不够标准哟！」。
...
```python
status_code = int(input('响应状态码: '))   # 输入: 404
match status_code:
...
print('状态码描述:', description)   # 输出: 状态码描述: Not Found
```
...
if x > 1:
    ...
    y = 5 * x + 3
print(f'{y = }')   # 输出: y = 1.0（输入 0.5 则输出 y = 2.5，输入 -2 则输出 y = -7.0）
```
...
if x > 1:
    ...
    if x >= -1:
        ...
        y = 5 * x + 3
print(f'{y = }')   # 输出: y = 1.0（与上一版结果完全相同，只是写法不同）
```
...
if score >= 90:
    ...
    grade = 'E'
print(f'{grade = }')   # 输出: grade = 'B'（注意成绩是字符串，输出带引号）
```
#### 例子3：计算三角形的周长和面积。
...
Author: 骆昊
"""
a = float(input('a = '))   # 输入: 3
b = float(input('b = '))   # 输入: 4
c = float(input('c = '))   # 输入: 5
if a + b > c and a + c > b and b + c > a:
    perimeter = a + b + c
    print(f'周长: {perimeter}')   # 输出: 周长: 12.0
    s = perimeter / 2
    area = (s * (s - a) * (s - b) * (s - c)) ** 0.5
    print(f'面积: {area}')        # 输出: 面积: 6.0
else:
    print('不能构成三角形')   # 输入 1 / 2 / 5 时走这里
```
