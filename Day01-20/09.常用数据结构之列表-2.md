...
# 从索引位置3开始查找'Java'，找不到会报错
print(items.index('Java', 3))
```

> **最后一行会报错**，你看到的不是输出，而是这样一段信息：
>
> ```text
> Traceback (most recent call last):
>   File "C:\code\example.py", line 9, in <module>
>     print(items.index('Java', 3))
>           ~~~~~~~~~~~^^^^^^^^^^^
> ValueError: 'Java' is not in list
> ```
>
> 这段信息要**从下往上读**：最后一行是错误类型（`ValueError`）和原因；
> 往上第二段指出**哪个文件、第几行**出错，并抄出那一行代码；`^^^` 标出具体出问题的位置。
> 详细读法见[第一个Python程序](02.第一个Python程序.md)里的「读懂报错信息」一节。
...
for _ in range(5):
    temp = []
    for _ in range(3):
        score = int(input('请输入: '))   # 共需输入 5 名学生 × 3 门课 = 15 个数
        temp.append(score)
    scores.append(temp)
print(scores)   # 输入 60 70 80 / 90 85 75 / 100 95 88 / 72 66 80 / 55 60 70 时输出:
               # [[60, 70, 80], [90, 85, 75], [100, 95, 88], [72, 66, 80], [55, 60, 70]]
```
...
我们在 PyCharm 中运行上面的代码，输入`5`，会得到下面这样的输出。

```text
生成几注号码: 03 05 11 12 14 26 02
16 17 18 23 25 28 03
03 04 06 17 20 31 06
06 09 22 23 24 30 11
08 13 22 25 30 31 13
```

> **说明**：每行**前 6 个数字是红色、最后 1 个是蓝色**——这是代码里`\033[031m`和`\033[034m`两个控制码的效果。纯文本编辑器里看不到颜色，只能看到数字。上面的号码是随机生成的，你每次运行都会得到不同的结果：每行前 6 个是红球号码（从`1`到`33`中无放回抽取后排序），最后 1 个是蓝球号码（从`1`到`16`中随机抽取）。
...
Would install rich-15.0.0
```

> **说明**：上面是`pip install --dry-run rich`的真实输出——`--dry-run`表示只解析依赖、不真的安装，所以最后一行是`Would install`。去掉`--dry-run`真装时，最后一行会变成`Successfully installed rich-15.0.0`，看到这一行就说明装好了。如果提示`Requirement already satisfied`，说明这个库之前已经装过，不用再装。

如上所示安装好 rich 之后，我们可以用如下所示的代码来控制输出。
...
n = int(input('生成几注号码: '))   # 输入: 5（号码随机，每次运行都不同）
red_balls = [i for i in range(1, 34)]
blue_balls = [i for i in range(1, 17)]
